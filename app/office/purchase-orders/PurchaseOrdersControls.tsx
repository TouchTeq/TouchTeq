...
type POStatus = 'all' | 'draft' | 'sent' | 'acknowledged' | 'delivered' | 'cancelled';

const STORAGE_KEY = 'office:purchase-orders:filters:v1';

function buildUrl(pathname: string, next: { q: string; status: POStatus }) {
...
export default function PurchaseOrdersControls({
  initialQ,
  initialStatus,
}: {
  initialQ: string;
  ...
  const { load, save } = useSavedFilters<{ q: string; status: POStatus }>(STORAGE_KEY);

  const qParam = searchParams.get('q') ?? initialQ ?? '';
  const statusParam = (searchParams.get('status') as POStatus) ?? initialStatus ?? 'all';

  const defaults = useMemo(() => ({ q: '', status: 'all' as POStatus }), []);
  const [q, setQ] = useState(qParam);
  const [status, setStatus] = useState<POStatus>(statusParam);
  ...
  const apply = (next: { q: string; status: POStatus }) => {
    save(next);
    ...
    <div className="bg-[#151B28] border border-slate-800/50 p-4 rounded-xl space-y-4">
      <div className="flex flex-col lg:flex-row gap-4">
        ...
        <div className="flex bg-[#0B0F19] p-1 rounded-lg border border-slate-800/50 overflow-x-auto">
          {(
            ['all', 'draft', 'sent', 'acknowledged', 'delivered', 'cancelled'] as POStatus[]
          ).map((opt) => (
            <button
              key={opt}
              type="button"
