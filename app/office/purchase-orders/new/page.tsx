...
export default function NewPurchaseOrderPage() {
  ...
  const [supplierMode, setSupplierMode] = useState<"existing" | "quick">(
    "existing",
  );
  const [quickSupplier, setQuickSupplier] = useState<QuickClientData>({
    ...EMPTY_QUICK_CLIENT,
  });
  ...
  // When switching back to existing mode, clear quick supplier data
  useEffect(() => {
    if (supplierMode === "existing") {
      setQuickSupplier({ ...EMPTY_QUICK_CLIENT });
    }
  }, [supplierMode]);
  ...
  const handleSave = async (sendToSupplier: boolean = false) => {
    ...
    if (supplierMode === "existing" && !formData.supplier_name.trim()) {
      toast.error({
        title: "Required",
    ...
    if (supplierMode === "quick" && !quickSupplier.company_name.trim()) {
      toast.error({
        title: "Required",
        message: "Please enter a supplier / company name.",
      });
      return;
    }
    ...
    try {
      ...
      if (supplierMode === "quick") {
        ...
        let supplierName = quickSupplier.company_name.trim();
        let supplierContact = quickSupplier.contact_person || "";
        let supplierEmail = quickSupplier.email || "";

        if (quickSupplier.save_to_database) {
          // Create supplier record in clients table
          const { data: newSupplier, error: supplierError } = await supabase
            .from("clients")
            .insert([
              {
                company_name: supplierName,
                contact_person: quickSupplier.contact_person || null,
                email: quickSupplier.email || null,
                physical_address: quickSupplier.physical_address || null,
                vat_number: quickSupplier.vat_number || null,
                category: "Supplier",
              },
            ])
            .select()
          ...
          if (supplierError)
            throw new Error(
              `Failed to create supplier: ${supplierError.message}`,
            );
        ...
        const { data: result, error: rpcError } = await supabase.rpc(
          "create_purchase_order_with_items",
          {
            ...
            p_notes:
              [
                formData.notes || "",
                quickSupplier.physical_address
                  ? `Address: ${quickSupplier.physical_address}`
                  : "",
                quickSupplier.vat_number
                  ? `VAT: ${quickSupplier.vat_number}`
                  : "",
                quickSupplier.save_to_database
                  ? ""
                  : "(Supplier not saved to database)",
              ]
                .filter(Boolean)
                .join("\n") || null,
            p_date_raised: formData.date_raised,
            p_status: newStatus,
            p_supplier_contact: supplierContact || null,
        ...
        if (rpcError || !result)
          throw new Error(
            rpcError?.message || "Purchase order creation failed",
          );
        ...
        if (sendToSupplier && supplierEmail) {
          try {
            ...
            await sendDocumentEmail({
              ...
              recipientName: supplierContact || supplierName,
            });

            toast.success({
              title: "Sent",
              message: `PO ${poNumber} sent to ${supplierEmail}.`,
            });
          } catch (emailErr) {
            console.error("Email error:", emailErr);
            toast.error({
              title: "Email Failed",
              message: "PO saved but email delivery failed.",
            });
          }
        }
        ...
        // Existing supplier — original flow
        const { data: result, error: rpcError } = await supabase.rpc(
          "create_purchase_order_with_items",
          {
        ...
        if (sendToSupplier && formData.supplier_email) {
          try {
            ...
            await sendDocumentEmail({
              ...
              recipientName:
                formData.supplier_contact || formData.supplier_name,
            });

            toast.success({
              title: "Sent",
              message: `PO ${poNumber} sent to ${formData.supplier_email}.`,
            });
          } catch (emailErr) {
            console.error("Email error:", emailErr);
            toast.error({
          ...
          <div className="bg-[#151B28] border border-slate-800/50 p-6 rounded-xl">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xs font-black uppercase text-slate-500 flex items-center gap-2">
                Supplier Details
                {supplierId && supplierMode === "existing" && (
                  <button
                    type="button"
              ...
              <ClientModeToggle
                ...
                existingLabel="Existing Supplier"
                quickLabel="Quick Add Supplier"
              />
            </div>
            ...
            {supplierMode === "existing" ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2 md:col-span-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleFieldChange("supplier_email", e.target.value)
                      }
              ...
              <QuickClientForm
                value={quickSupplier}
                onChange={setQuickSupplier}
                nameLabel="Supplier / Company Name"
                namePlaceholder="Enter supplier or company name"
                type="supplier"
              />
            )}
          </div>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (val === "") {
                              ...
                              if (!isNaN(num))
                                updateLineItem(idx, "quantity", num);
                            }
                          }}
                          onBlur={(e) => {
                            const val = e.target.value;
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (val === "") {
                              ...
                              if (!isNaN(num))
                                updateLineItem(idx, "unit_price", num);
                            }
                          }}
                          onBlur={(e) => {
                            const val = e.target.value;
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <select
                  ...
                  onChange={(e) =>
                    handleFieldChange("linked_quote_id", e.target.value)
                  }
                  className="w-full bg-[#0B0F19] border border-slate-800 rounded-lg px-4 py-2 text-white text-sm"
                >
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  onChange={(e) =>
                    handleFieldChange("linked_invoice_id", e.target.value)
                  }
                  className="w-full bg-[#0B0F19] border border-slate-800 rounded-lg px-4 py-2 text-white text-sm"
                >
                  <option value="">None</option>
