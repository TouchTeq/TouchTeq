...
function NewQuoteContent() {
  ...
  useEffect(() => {
    async function init() {
      ...
      const nextValidity = Number.isFinite(configuredValidity)
        ? Math.max(1, Math.min(365, configuredValidity))
        : 30;
      ...
      setFormData((prev) => ({
        ...prev,
        expiry_date: format(
          addDays(new Date(`${prev.issue_date}T00:00:00`), nextValidity),
          "yyyy-MM-dd",
        ),
      }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (clientMode === "quick") {
        ...
        if (rpcError || !result)
          throw new Error(rpcError?.message || "Quotation creation failed");
            ...
            <div className="p-8 flex flex-col md:flex-row justify-between items-start gap-8">
              ...
              <div className="w-full md:w-96 md:ml-auto space-y-3 pt-6 border-t md:border-t-0 md:pt-0 border-slate-800">
                ...
                <div className="flex justify-between items-center py-4 border-t border-slate-800">
                  ...
                  <span className="text-xl font-black text-orange-500 text-right min-w-[120px]">
                    {formatRand(total)}
                  </span>
                </div>
              </div>
          ...
          <div className="bg-[#151B28] border border-slate-800/50 rounded-xl overflow-visible shadow-2xl p-6 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-600">
                Default validity: {quoteValidityDays} days
              </p>
            </div>
