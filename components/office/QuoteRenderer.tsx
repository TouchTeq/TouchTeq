...
import { buildQuoteRenderData, formatQuoteCurrency } from '@/lib/quotes/quote-renderer-data';
...
function SupplierBrand({ logoUrl, supplierName, accentColor }: { logoUrl?: string; supplierName?: string; accentColor: string }) {
  ...
  const name = supplierName || 'Quotation';
  return (
    <div className="flex items-center gap-3">
      ...
      <div>
        <p className="text-lg font-black tracking-tight text-slate-950">{name}</p>
...
export default function QuoteRenderer({ quote, lineItems, businessProfile, className = '' }: QuoteRendererProps) {
  const data = buildQuoteRenderData(quote, lineItems, businessProfile);
    ...
    >
      <div className="flex items-start justify-between border-b pb-8" style={{ borderColor: accentColor }}>
        ...
        <div className="text-right">
          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Quotation</p>
          {data.quoteNumber && (
      ...
      <div className="mt-8 grid grid-cols-[1.1fr_0.9fr] gap-10">
        ...
        <section className="space-y-6">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.28em] text-slate-400">Prepared For</p>
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50 px-5 py-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Valid Until</p>
                <p className="mt-1 text-sm font-black text-slate-900">{data.expiryDate || '—'}</p>
              </div>
            </div>
          </div>
        </section>
            ...
            {data.lineItems.map((item, index) => (
              <tr key={`${item.description}-${index}`} className="border-t border-slate-100 align-top">
                ...
                <td className="px-4 py-4 text-right text-xs font-bold text-slate-500">
                  R {formatQuoteCurrency(item.unitPrice)}
                </td>
                <td className="px-6 py-4 text-right text-xs font-black text-slate-900">
                  R {formatQuoteCurrency(item.lineTotal)}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <div className="mt-8 flex justify-end">
        <div className="w-[320px] space-y-3">
          <div className="flex items-center justify-between text-xs font-bold uppercase tracking-[0.18em] text-slate-500">
            <span>Subtotal</span>
            <span>R {formatQuoteCurrency(data.subtotal)}</span>
          </div>
          <div className="flex items-center justify-between text-xs font-bold uppercase tracking-[0.18em] text-slate-500">
            <span>VAT (15%)</span>
            <span>R {formatQuoteCurrency(data.vatAmount)}</span>
          </div>
          <div className="flex items-end justify-between gap-6 border-t border-slate-200 pt-4">
            ...
            <span className="text-2xl font-black tracking-tight" style={{ color: accentColor }}>
              R {formatQuoteCurrency(data.total)}
            </span>
          </div>
        </div>
      </div>
      ...
      {data.bankDetails.length > 0 && (
        <section className="mt-10 rounded-2xl border border-slate-200 bg-slate-50 px-6 py-6">
          <h2 className="text-[11px] font-black uppercase tracking-[0.28em] text-slate-900">Banking Details</h2>
      ...
      <footer className="mt-10 border-t border-slate-200 pt-6 text-center">
        ...
        <p className="mt-2 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500">
          This quotation is valid until {data.expiryDate || 'the date shown above'}.
        </p>
      </footer>
    </div>
  );
